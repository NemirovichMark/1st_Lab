...
#endregion
#region Excersice first level number 9 
double sum = 0;
double s = 1.00;
int pow3 = 1, pow2 = 1;
double pow1 = 1;
...
#endregion 
#region Excersice first level number 15
int n = 5;
int first = 1, secound = 1, next;
for (int i = 1; i <= n; i++)
{
    if (i <= 1)
    {
...
#endregion
#region Excersice first level number 16
...
#endregion
#region Excersice first level number 18 
int cells = 10;
...
#endregion
#region Excersice secound level number 2
int pp = 1;
while (pp * (pp + 3) < 30000)
{
    pp += 3;
...
if (x_base < 1 && x_base > -1)
{
    ...
    double path = 10000;
    double increasedPathPerDay = 0.10;
    for (int i = 1; i <= 7; i++)
    {
    ...
    Console.WriteLine($"in {days} days the runner runs more than 20 km per day"); }
#endregion
